...
function fFinalJuego () {
    ...
    if (partida.parejas === 8) {
        partida.resultado = "won"
    } else {
        partida.resultado = "lost"    
    }
    ...
    //alert("End Game \n You " + partida.resultado + "!!!")    
    fModal("You " + partida.resultado);
...
function fModal(pResultado) {

    console.log(pResultado);
    document.querySelector(".popup").style.display = "block";

    document.getElementById("popupfinal").innerHTML = pResultado;
...
function fDificultadInicio () {
    ...
    for(let idx=0 ; idx< combo.length; idx++) {
        if (combo[idx].checked === true) {
            ...
            partida.tiempo = combo[idx].dataset.tiempo                        
        }
